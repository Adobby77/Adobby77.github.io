...
Here, $$\dot h(x,u)=\frac{\pp h}{\pp x}\frac{dx}{dt}=\frac{\pp h}{\pp x}\left(f(x)+g(x)u\right)=\underbrace{\frac{\pp h}{\pp x}f(x)}_{L_f h(x)}+\underbrace{\frac{\pp h}{\pp x}g(x)}_{L_g h(x)}u=L_fh(x)+L_gh(x)u$$
...
##### Thm 2.4) Safety guarantee via CBFs (sufficiency for safety)
...
In some ideal cases, it may possible to take $h(x)=\rho(x)$, i.e. $$\forall x\in A,\;\exists u\in U\q\text{s.t.}\q\dot\rho(x,u)\geq -\al(\rho(x))$$ Then, $\mathcal{C}=A$ and hence $A$ is forward invariant itself as well.
...
> 	- In many cases, $U$ is defined by $$U=\{u\in\R^m:\lnm u\rnm\leq u_\max\}$$
> 	- $\rho=h$ implies that $\rho$ is CBF and hence $A$ is forward invariant. So, by the definition of the set of all safe controllers([[#Def 2.4) Modern definition of Control Barrier Functions, Set of all safe controllers|definition 2.4]]), the set of control inputs that render $A$ invariant is
...
> 	- Let $\rho(x)$ has a relative degree $r$. If $r\geq 2$, then by the definition of  [[Nonlinear systems#Def) Relative degree|relative degree]],
...
> 	- However, as we observed in [[#2.3.1. Safety-Critical Control|CBF-QP]],  we need to compute the gradient of $h$ to utilize $h$ in QP problem that requires **huge computational burden as the dimension of the system grows.**
...
In short,
- [[#Thm 3.3) CBF using nominal controller|Theorem 3.3]] constructively provides the theoretical existence of CBF.
- [[Control Barrier Function#Prop 3.5) Polynomial CBF Synthesis via SOS|Proposition 3.5]] provides the way to compute the CBF constructed in theorem 3.3.

However, there is one more problem left.

While SDP solvers can only deal with "Linear" constraints, the SOS constraints in the method proposed in [[#Prop 3.5) Polynomial CBF Synthesis via SOS|proposition 3.5]] are **"Bilinear"**(observe $ah(x)$ and $s_2(x)h(x)$).
...
First, recall the definition [[Nonlinear systems#Def) Relative degree|relative degree]].
...
> \end{bmatrix}\in\R^{r}
> $$
> Also, by assuming a state-feedback controller $\mu=-K_\al\eta_b(x)$, then we $$h(x(t))=Ce^{(F-GK_\al)}\eta_b(x_0)$$
> 
> ---
...
> $$
> Moreover, by the [[#Lem 2.1) Comparison lemma|comparison lemma]], if $\mu\geq -K_\al\eta_b(x)$, then $h(x(t))\geq Ce^{(F-GK_\al)}\eta_b(x_0)$. <span style="float: right;">$\square$</span>
...
**Q. Is this definition properly generalizes the definition of [[#Def 2.4) Modern definition of Control Barrier Functions, Set of all safe controllers|CBF]]?**
...
> Suppose $K_\al$ is chosen s.t. $F-GK_\al$ is Hurwitz and total negative(resulting in negative real poles) and the eigenvalues satisfy $$\lambda_i(F-GK_\al)\geq -\frac{\dot\nu_{i-1}(x_0)}{\nu_{i-1}(x_0)},$$then $\mu\geq -K_\al\eta_b(x)$ guarantees $h(x)$ is an exponential CBF.
