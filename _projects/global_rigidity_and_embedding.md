...
We consider formations defined by a graph with prescribed pairwise distance constraints.

Let \( G = (V, E) \) be a finite undirected graph with \( |V| = m \).  
Let \( d : E \to \mathbb{R}_{>0} \) assign a positive length to each edge.  

A **framework** in a metric space \( (X, \rho) \) is a map  
\[
p : V \to X
\]
such that for every edge \( (i,j) \in E \),
\[
\rho(p(i), p(j)) = d_{ij}.
\]

The pair \( (G,d) \) will be called a **formation**.

Two frameworks \( p, q : V \to X \) are said to be **congruent** if there exists an isometry \( \Phi : X \to X \) such that
\[
q(v) = \Phi(p(v)) \quad \forall v \in V.
\]
...
- Euclidean space \( \mathbb{R}^n \), or  
- a smooth Riemannian manifold \( \mathcal{M} \).

A formation \( (G,d) \) is said to be **globally rigid in \( X \)** if for any two frameworks  
\( p, q : V \to X \) satisfying the distance constraints,  
